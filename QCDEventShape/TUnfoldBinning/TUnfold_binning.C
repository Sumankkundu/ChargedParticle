...
void TUnfold_binning(){
  ...
  TFile *outputFile=new TFile("TUnfoldBinning_retest.root","recreate");
  const char* vartitle[nvar]={"Anti-Y_{23,C} ", "Anti-Y_{23,E} ", "Anti-Y_{23,R} ",
   ...
   int htnbins[nHLTmx] = {10,30,50,70,50,50,50,200};
...
{-5.89, -5.42, -4.95, -4.49, -4.05, -3.63, -3.23, -2.86, -2.51, -2.18, -1.86, -1.55, -1.25}},//12
  ...
  TH1* h_recoevtvar[type][nusedvar][nHLTmx];
  //For fake
  TH1* h_recoevtfake[type][nusedvar][nHLTmx];
  //For Miss
  TH1* h_genevtmiss[type][nusedvar][nHLTmx];

  TH1* h_genevtvar[type][nusedvar][nHLTmx];
 ...
 for(int ity=0; ity <type; ity++){
   for(int ivar=0; ivar < nusedvar ; ivar++){
     for(int ipt = 0 ; ipt < njetptmn ; ipt++){
      ...
      RecoBinning[ity][ivar][ipt]->AddAxis(Axisname, (ity==0) ? nbinsx0[var[ivar]][ipt] : nbinsx1[var[ivar]][ipt], (ity==0) ? binrngs0[var[ivar]][ipt]:binrngs1[var[ivar]][ipt], true, true);

      sprintf(Axisname, "%s", "H_{T,2}");
      RecoBinning[ity][ivar][ipt]->AddAxis(Axisname, htnbins[ipt], htbins[ipt], htbins[ipt+1], false, false);
     }
   }
 }
...
for(int ity=0; ity <type; ity++){
   for(int ivar=0; ivar < nusedvar ; ivar++){
     for(int ipt = 0 ; ipt < njetptmn ; ipt++){
      ...
      sprintf(Axisname, "%s", "H_{T,2}");
      GenBinning[ity][ivar][ipt]->AddAxis(Axisname, htnbins[ipt], htbins[ipt], htbins[ipt+1], false, false);
     }
   }
 }
...
//Create and book histograms for reco  and gen
 ...
 //     binsGen[ity][ivar][ipt]->Write();
      ...
      sprintf(Axisname, "%s", vartitle[var[ivar]]);
       ...
       RM[ity][ivar][ipt] = TUnfoldBinning::CreateHistogramOfMigrations(binsGen[ity][ivar][ipt], binsRec[ity][ivar][ipt], histname ,0,0, Axisname); 
      //RM[ity][ivar][ipt] = TUnfoldBinning::CreateHistogramOfMigrations(binsRec[ity][ivar][ipt], binsGen[ity][ivar][ipt],histname);
...
sprintf(histname, "corr_typ_%i_pt%i_eta0_%i", 0, 0, 0);
TH2 *histMCGenRec[type][nusedvar][nHLTmx];
/*
histMCGenRec[0][0][0]=TUnfoldBinning::CreateHistogramOfMigrations(binsRec[0][0][0],binsGen[0][0][0], histname);
histMCGenRec[0][0][0]->Write();

sprintf(histname, "corr_typ_%i_pt%i_eta0_%i", 1, 6, var[4]);
histMCGenRec[1][4][6]=TUnfoldBinning::CreateHistogramOfMigrations(binsRec[1][4][6],binsGen[1][4][6], histname);
histMCGenRec[1][4][6]->Write();

sprintf(histname, "corr_typ_%i_pt%i_eta0_%i", 0, 6, var[4]);
histMCGenRec[0][4][6]=TUnfoldBinning::CreateHistogramOfMigrations(binsRec[0][4][6],binsGen[0][4][6], histname);
histMCGenRec[0][4][6]->Write();

sprintf(histname, "corr_typ_%i_pt%i_eta0_%i", 1, 7, var[2]);
histMCGenRec[1][2][7]=TUnfoldBinning::CreateHistogramOfMigrations(binsRec[1][2][7],binsGen[1][2][7], histname);
histMCGenRec[1][2][7]->Write();
*/

for(int ity=0; ity <type; ity++){
   for(int ivar=0; ivar < nusedvar ; ivar++){
     for(int ipt = 0 ; ipt < njetptmn ; ipt++){
sprintf(histname, "RM_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
cout << " Histname = " <<histname <<endl; 
//TH2 *rm=TUnfoldBinning::CreateHistogramOfMigrations(binsRec[ity][ivar][ipt],binsGen[ity][ivar][ipt], histname);
histMCGenRec[ity][ivar][ipt]=TUnfoldBinning::CreateHistogramOfMigrations(binsRec[ity][ivar][ipt],binsGen[ity][ivar][ipt], histname);
histMCGenRec[ity][ivar][ipt]->Write();

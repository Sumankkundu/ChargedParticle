...
*/
//for Gen bin same as Reco
//For Jet 
/*const int nmxbins=26;
...
//TUnfold Binning Test
/*
void TUnfoldCreateBinning(){


    
 TUnfoldBinning * detector, * generator;
 TUnfoldBinning *detectorDistribution = detector->AddBinning("detectordistribution");
...
double endxt[nvar]={8.0, 8.0, 6.0, 8.0, 6.0,  5.0,
                    ...
                    0.0, 0.0};
double startx[nvar]={2., 0.5, 1., 1.0, 0.0, 2.,
                     0., 0., 1., 0.2, -1., -1.,
                     0., 0., 1., 1.0, 2., 0.5,
                     0.2, -0.5, -1., 0., -1., -1.,
                     1.0, 0., -0.5, 0., 0., -0.5,
...
//    myfile.open("txt.log");
  ...
  //For fake 
  TH1F* h_recoevtfake[ntype][njetptmn][njetetamn][nvar];
  //For Miss
  TH1F* h_genevtmiss[ntype][njetptmn][njetetamn][nvar];
...
//Define the Variable for branchi
  ...
  bool isreco, isgen;
//   Double_t weight;  
 vector<Double_t>  EJet, EChar, EJetgen, EChargen;
...
//----------------------TTree for EventShape --------
 //TEsv = new TTree("TEsv", "QCDEvt");
 TEsv = fs->make<TTree> ("TEsv", "QCDEvt");

  TEsv->Branch("EJet", "vector<Double_t>", &EJet);
  TEsv->Branch("EJetgen", "vector<Double_t>", &EJetgen);
  TEsv->Branch("EChar", "vector<Double_t>", &EChar);
  TEsv->Branch("EChargen", "vector<Double_t>", &EChargen);
  TEsv->Branch("isreco", &isreco, "isreco/O");
  TEsv->Branch("isgen", &isgen, "isgen/O");
  TEsv->Branch("ht2", &ht2, "ht2/F");
  TEsv->Branch("ht2gen", &ht2gen, "ht2gen/F");
  TEsv->Branch("eweight", &eweight, "eweight/F");
	...
	for (int ij=0; ij<nvar; ij++) {
	  if (isItUsed(ij)) { 
	   ...
	   //h_recoevtvar[ityp][ipt][iet][ij] = fs->make<TH1F>(name, title, (ityp==0) ? nbinsx0[ij] : nbinsx1[ij], (ityp==0) ? binrngs0[ij] : binrngs1[ij] );  
           h_recoevtvar[ityp][ipt][iet][ij] = fs->make<TH1F>(name, title, (ityp==0) ? nbinsx0[ij][ipt] : nbinsx1[ij][ipt], (ityp==0) ? binrngs0[ij][ipt] : binrngs1[ij][ipt] );//Reco each HT,var
...
//	    h_genevtvar[ityp][ipt][iet][ij] = fs->make<TH1F>(name, title, (ityp==0) ? nbinsx0[ij] : ((ityp==1 || ityp==3) ? nbinsx1[ij] : nbinsx2[ij]), (ityp==0) ? binrngs0[ij] : ((ityp==1 || ityp==3) ? binrngs1[ij] : binrngs2[ij]));
        ...
        //  h_2devtvar[ityp][ipt][iet][ij] = fs->make<TH2F>(name, title, (ityp==0) ? nbinsx0[ij] : ((ityp==1 || ityp==3) ? nbinsx1[ij] : nbinsx2[ij]), (ityp==0) ? binrngs0[ij] : ((ityp==1 || ityp==3) ? binrngs1[ij] : binrngs2[ij]), (ityp==0) ? nbinsx0[ij] : ((ityp==1 || ityp==3) ? nbinsx1[ij] : nbinsx2[ij]), (ityp==0) ? binrngs0[ij] : ((ityp==1 || ityp==3) ? binrngs1[ij] : binrngs2[ij])); //Old Analysis
...
ht2gen = avegenptres[0]; 
ht2 = aveleadingptjec[0];
EJet.clear();
EJetgen.clear();
EChar.clear();
EChargen.clear();
...
//if(itp==0){ cout <<" Var :  " << ij <<" : "<< recovar[ij];}
			...
			if(itp==0){ EJet.push_back(recovar[ij]); isreco =true;}
			if(itp==1){ EChar.push_back(recovar[ij]); isreco =true;}
                         }
		...
		if (genvar[nvar]>=2) {
		  ...
		  for (int ij=0; ij<nvar; ij++) {
			...
			h_genevtvar[itp][igenhtres[isrc]][iet][ij]->Fill(genvar[ij], weighttrg);
                        if(itp==0){ EJetgen.push_back(genvar[ij]); isgen =true;}
                        if(itp==1){ EChargen.push_back(genvar[ij]); isgen =true;}
...
if(nevt==100){   cout <<igenht <<endl;}
   ...
   // cout <<"end event" << endl;
if(isreco ==true || isgen==true)  { TEsv->Fill();}
}

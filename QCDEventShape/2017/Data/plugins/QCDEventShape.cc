...
// h_recoevtvarres[ityp][ipt][iet][ij][ix] = fs->make<TH1F>(name, title, (ityp==0) ? nbinsx0[ij] : ((ityp==1 || ityp==3) ? nbinsx1[ij] : nbinsx2[ij]), (ityp==0) ? binrngs0[ij] : ((ityp==1 || ityp==3) ? binrngs1[ij] : binrngs2[ij]));
	    ...
	    sprintf(title, "Gen %s %i %g %s", typname[ityp], int(leadingPtThreshold[ipt]), etarange[iet], vartitle[ij]);
            ...
            h_genevtvar[ityp][ipt][iet][ij] = fs->make<TH1F>(name, title, (ityp==0) ? nbinsx0[ij][ipt] : nbinsx1[ij][ipt], (ityp==0) ? binrngs0[ij][ipt] : binrngs1[ij][ipt]);//Bins for each HT
   ...
   //   double CEMF = (*ak4PFJets)[ij].chargedEmEnergyFraction();
    ...
    //  int NumNeutralParticles =(*ak4PFJets)[ij].neutralMultiplicity();
      ...
      if(abs((*ak4PFJets)[ij].eta())<=2.7){
				if( (NHF<0.99 && NEMF<0.99 && NumConst>1) && (abs((*ak4PFJets)[ij].eta())<=2.4 && CHF>0 && CHM>0 ))  TightJetID =true;      
                   } else {
			TightJetID =false;
      }
   ...
   //res_sf = JME::JetResolutionScaleFactor("Fall15_25nsV2_MC_SF_AK4PFchs.txt");
	...
	//	int NumNeutralParticles =(*ak4PFJets)[ireorjt].neutralMultiplicity();
		...
		if(abs((*ak4PFJets)[ireorjt].eta())<=2.7){
		  if( (NHF<0.99 && NEMF<0.99 && NumConst>1) && (abs((*ak4PFJets)[ireorjt].eta())<=2.4 && CHF>0 && CHM>0)) TightJetID =true;
		                 }
...
//  t1=clock();
 ...
 //JetCorrectorParameters *p = new JetCorrectorParameters("Fall17_17Nov2017E_V6_DATA_UncertaintySources_AK4PFchs.txt", name);    
 //JetCorrectorParameters *p = new JetCorrectorParameters("Fall17_17Nov2017D_V6_DATA_UncertaintySources_AK4PFchs.txt", name);    
 //JetCorrectorParameters *p = new JetCorrectorParameters("Fall17_17Nov2017C_V6_DATA_UncertaintySources_AK4PFchs.txt", name);    
 //JetCorrectorParameters *p = new JetCorrectorParameters("Fall17_17Nov2017B_V6_DATA_UncertaintySources_AK4PFchs.txt", name);    

 //  JetCorrectorParameters *p = new JetCorrectorParameters("Summer19UL17_RunB_V4_DATA_UncertaintySources_AK4PFchs.txt", name);    
 //JetCorrectorParameters *p = new JetCorrectorParameters("Summer19UL17_RunC_V4_DATA_UncertaintySources_AK4PFchs.txt", name);    
 //JetCorrectorParameters *p = new JetCorrectorParameters("Summer19UL17_RunD_V4_DATA_UncertaintySources_AK4PFchs.txt", name);    
 //JetCorrectorParameters *p = new JetCorrectorParameters("Summer19UL17_RunE_V4_DATA_UncertaintySources_AK4PFchs.txt", name);    

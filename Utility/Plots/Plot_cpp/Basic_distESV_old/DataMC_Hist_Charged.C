...
//   sprintf(Title,"  %s %s",htrang[iplot] ," GeV" );
 // sprintf(Xaxis," %s" ,Esvlogx[ivar]);
 if(iplot<=6){ sprintf(Title," Reco Charged Particles:  %s      %i <H_{T,2}< %i %s",Esvsym[ivar] , HT2range[iplot] , HT2range[iplot+1] ,"GeV/c" );}
 else if(iplot==7){ sprintf(Title,"Reco Charged Particles :  %s      H_{T,2} > %i %s",Esvsym[ivar] ,  HT2range[iplot] ,"GeV/c" );}
...
//lowpadx = x axis title of the ratio plot
   ...
   if(ivar==0 && iplot ==0){cpt0->Print("Esv_dataMC_charged.pdf(","pdf");
   }else if(ivar==4 && iplot==7) {cpt0->Print("Esv_dataMC_charged.pdf)","pdf");
   }else{
     cpt0->Print("Esv_dataMC_charged.pdf","pdf");};

...
void Unfoldplot(){
  ...
  static const int unfold_ty =3; //Number of Unfold 
  ...
  char histname[100],Title[100], Xaxis[100], Yaxis[100], ratioXaxis[100], ratioYaxis[100],pdfname[100],pdfname1[100],pdfname2[100];
  bool Reco, Gen;
  ...
  const  char* unfoldty[3]={"bayes_unfold_","svd_unfold_","BbB_unfold_"};
  const  char* reg_refold[3]={"unfold_back_bayes_","unfold_back_SVD_","unfold_back_BbB_"};
  const  char* CORR[3]={"Global_correl2D_coeff_bayes_unfold_","Global_correl2d_coeff_svd_unfold_","Global_correl2d_coeff_BbB_unfold_"};
  const  char* COVN[3]={"EMatrix_bayes_unfold_","Ematrix_svd_unfold_","Ematrix_BbB_unfold_"};
  const  char* ProbN[3]={"Cov_bayes_unfold_","probMatrix_svd_unfold_","Probability_BbB_unfold_"};

  const  char* Validity_test[4]={"Closure test","Bottom Line test"," Unfolded","Refold"};
  const  char* h2dMat_name[4]={"Covariance matrix","Correlation coefficients"," Matrix of probabilities ","Response matrix"};
  const  char* mcname[3]={"Pythia8 CP5 Tune","Madgraph","Herwig++"};
  const  char* mcnamerco[3]={"Reco Pythia8 ","Reco Madgraph","Reco Herwig++"};
  const  char* mcnamegen[3]={"Pythia8 Gen","Madgraph Gen","Herwig++ Gen"};
  const  char* itypeN[ntype]={"Jets","Charged Particles"}; 
  const  char* DataEra[3]={"Data 2016","Data 2017","Data 2018"};
  const  char* Datarec[3]={"Data reco","Data reco","Data reco"};
  const  char* MCrec[3]={"Pythia8 reco","Madgraph reco","Herwigg++ reco"};
  const  char* UndoldEra[3]={"Unfold 2016","Unfold 2017","Unfold 2018"};
  const  char* RefoldEra[3]={"Refold PY8(Bayes)","Refold PY8(SVD)","Refold PY8 (Bin by Bin)"};
  const  char* Unfoldtype[5]={"Roounfold(bayes)","Roounfold(SVD)","Roounfold(bin by bin)","Unfolded(Scan Tau)","Unfolded(Iterative method)"};
  const  char* closuretype[5]={"Roounfold PY8(bayes)","Roounfold PY8(SVD)","Roounfold PY8(bin by bin)","Unfolded(Scan Tau)","Unfolded(Iterative method)"};
  static const int iera = 1;
  int iPeriod = 0;  int iPos=10 ;
  
  //******************************************************************************
  TFile *Unfoldroot = TFile::Open("/home/suman/Paradox/Charged_ESV/Working/Unfolding/RooUnfold_ESV/RooUnfold/Roounfold_closure.root");  // Unfolded data 
//TFile *Unfoldroot = TFile::Open("Roounfold_closure.root");  // Unfolded data 
  
  //--------------------------------------Function declear
  void Integralhist(TH1D *hist);
  void divBybinWidth(TH1D *hist);
  void Myplotset(TH1D *Myhist,const char* XTitle, const char* YTitle);
  void SetMycanvas(TCanvas *cpt);
  void CTLegend( TLegend *legendn ,double  x1,double y1,double x2,double y2,const char* txt);
  ...
  for(int  imc =0; imc < nmc ; imc++){
	...
	for(Int_t ipt =0; ipt < nHLTmx ; ipt++){     
#ifdef REBIN
	  sprintf(histname, "RebinMC/rebin_gen_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]); //reco_typ_1_pt6_eta0_15
#else
	  sprintf(histname, "MC/gen_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]); //reco_typ_1_pt6_eta0_15
#endif
...
#ifdef REBIN
	  sprintf(histname, "RebinMC/rebin_reco_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]); //reco_typ_1_pt6_eta0_15
#else
	  sprintf(histname, "MC/reco_typ_%i_pt%i_eta0_%i", ity,  ipt, var[ivar]); //reco_typ_1_pt6_eta0_15
#endif
	  ...
	  //Read Reso
#ifdef REBIN
	  sprintf(histname, "RebinMC/Rebin_corr_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]); //reco_typ_1_pt6_eta0_15
#else
	  sprintf(histname, "MC/corr_typ_%i_pt%i_eta0_%i", ity,  ipt, var[ivar]); //reco_typ_1_pt6_eta0_15
#endif
	  MC_Res[imc][ity][ivar][ipt] = (TH2D*) Unfoldroot->Get(histname);
	  cout << histname << endl;
	  
	  //------------------------------------------------------
	  sprintf(histname,"Unfold/Effi_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
	  hist_eff[imc][ity][ivar][ipt] = (TH1D*) Unfoldroot->Get(histname);
	  sprintf(histname,"Unfold/Fake_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
	  hist_fake[imc][ity][ivar][ipt] = (TH1D*) Unfoldroot->Get(histname);
	  sprintf(histname,"Unfold/Purity_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
  ...
  for(int ity=0; ity <ntype; ity++){
    for(int ivar =0 ; ivar < nusedvar ; ivar++){
      for(Int_t ipt =0; ipt < nHLTmx ; ipt++){
#ifdef REBIN
	sprintf(histname, "RebinData/rebin_reco_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]); //reco_typ_1_pt6_eta0_15
#else
  ...
  for(int iun=0; iun < unfold_ty; iun++){
	...
	for(int ipt = 0 ; ipt < nHLTmx ; ipt++){
	  sprintf(histname, "Unfold/%s%i_pt%i_eta0_%i",unfoldty[iun],ity,ipt, var[ivar]); //Tunfolded_typ_0_pt0_eta0_3
  ...
  //PLot canvas declear
  TCanvas *cpt0 = new TCanvas("cpt0", "canvas0", 700,600 );  //for ESVs
  TCanvas *cpt1 = new TCanvas("cpt1", "canvas1", 600,600 );  //for ESVs
  TCanvas *cpt2 = new TCanvas("cpt2", "canvas2", 600,600 );  //for ESVs
  TCanvas *cpt3 = new TCanvas("cpt3", "canvas3", 600,600 );  //for ESVs
  TCanvas *cpt4 = new TCanvas("cpt4", "canvas4", 800,800 );  //for ESVs
  TCanvas *cpt5 = new TCanvas("cpt5", "canvas5", 800,800 );  //for ESVs
  TCanvas *cpt6 = new TCanvas("cpt6", "canvas6", 800,800 );  //for ESVs
  TCanvas *cpt7 = new TCanvas("cpt7", "canvas7", 800,800 );  //for ESVs
  TCanvas *cpt8 = new TCanvas("cpt8", "canvas8", 800,800 );  //for ESVs
  ...
  for(int ity=0; ity <ntype; ity++){
	...
	if(ipt<=6){sprintf(Title,"%s: %s     %i <H_{T,2}< %i %s",itypeN[ity], Esvsym[ivar] , HT2range[ipt] , HT2range[ipt+1] ,"GeV/c" );}
	else if(ipt==7){ sprintf(Title,"%s:  %s      H_{T,2} > %i %s",itypeN[ity], Esvsym[ivar] ,  HT2range[ipt] ,"GeV/c" );}
...
//Response Matrix------------------------------------------------------
  for(int ity=0; ity <ntype; ity++){
    for(int ivar =0 ; ivar < nusedvar ; ivar++){
      for(int ipt = 0 ; ipt <nHLTmx; ipt++){
	cpt8->cd();//SetMycanvas(cpt8);	
        MC_Res[0][ity][ivar][ipt]->Draw("colz");
        MC_Res[0][ity][ivar][ipt]->Draw("colz");
        MC_Res[0][ity][ivar][ipt]->Draw("colz");
        TLegend *leg1 = new TLegend(0.2,0.8,0.6,0.8);
        leg1->SetFillStyle(0);
        ...
        leg1->AddEntry((TObject*)0, h2dMat_name[3], "");
        leg1->AddEntry((TObject*)0,Title , "");
	leg1->Draw();
	sprintf(pdfname, "Response_Mat.pdf("); sprintf(pdfname1, "Response_Mat.pdf"); sprintf(pdfname2, "Response_Mat.pdf)"); //TData_recoed_typ_0_pt0_eta0_3
...
//	SetMycanvas(cpt2);
  ...
  for(int iun=0; iun < unfold_ty; iun++){
	...
	for(int ipt = 0 ; ipt <nHLTmx; ipt++){
	  ...
	  sprintf(pdfname, "Unfold_plot_%i.pdf(" ,iun); sprintf(pdfname1, "Unfold_plot_%i.pdf" ,iun);sprintf(pdfname2, "Unfold_plot_%i.pdf)" ,iun); //Tunfolded_typ_0_pt0_eta0_3
	  if(ity==0 && ivar==0 && ipt ==0){cpt0->Print(pdfname,"pdf");
	  }else if(ity==1 && ivar==4 && ipt==7) {cpt0->Print(pdfname2,"pdf");
      ...
      // Corr[iun][ity][ivar][ipt]->Draw("colz");
       sprintf(pdfname, "Unfold_corr_%i.pdf(" ,iun); sprintf(pdfname1, "Unfold_corr_%i.pdf" ,iun);sprintf(pdfname2, "Unfold_corr_%i.pdf)" ,iun); //Tunfolded_typ_0_pt0_eta0_3
       ...
       cpt6->cd();
      // Prob[iun][ity][ivar][ipt]->Draw("colz");
       sprintf(pdfname, "Unfold_prob_%i.pdf(" ,iun); sprintf(pdfname1, "Unfold_prob_%i.pdf" ,iun);sprintf(pdfname2, "Unfold_prob_%i.pdf)" ,iun); //Tunfolded_typ_0_pt0_eta0_3
       ...
       cpt7->cd();
      // Ematrix[iun][ity][ivar][ipt]->Draw("colz");
       sprintf(pdfname, "Unfold_COV_%i.pdf(" ,iun); sprintf(pdfname1, "Unfold_COV_%i.pdf" ,iun);sprintf(pdfname2, "Unfold_COV_%i.pdf)" ,iun); //Tunfolded_typ_0_pt0_eta0_3
  ...
  cout <<" UNFOLD PLOT OK " << endl;
//All Unfold in one plot
    for(int ity=0; ity <ntype; ity++){
      for(int ivar =0 ; ivar < nusedvar ; ivar++){
        for(int ipt = 0 ; ipt <nHLTmx; ipt++){

          TH1D *MyHist  = (TH1D*) MC_gen[0][ity][ivar][ipt]->Clone();
          Integralhist(MyHist);
          divBybinWidth(MyHist);
          Myplotset(MyHist,0,0);
  ...
  for(int iun=0; iun < unfold_ty; iun++){
	...
	for(int ipt = 0 ; ipt <nHLTmx; ipt++){
	  ...
	  for(int iout = 0 ; iout < 1 ; iout++){
	    MC_input[iout] = (TH1D*) Data_reco[ity][ivar][ipt]->Clone();
	    Integralhist(MC_input[iout]);
	    divBybinWidth(MC_input[iout]);
#ifdef CLOUSER
	    MCinput_index[iout]= MCrec[0];}//mcname[iout]; }
#else
	    MCinput_index[iout]= Datarec[0];}//mcname[iout]; }
#endif
	  data_index[0]= RefoldEra[1];
	  char lplot_xtitle[100];
	  sprintf(lplot_xtitle, "%s",Esvlogx[ivar]);
	  int num1[2]={nmc,1};
	  float lpos1[7] ={.32,0.2,0.55,0.38, .04, 2.2,.4};
    ...
    for(int ity=0; ity <ntype; ity++){
      for(int ivar =0 ; ivar < nusedvar ; ivar++){
        for(int ipt = 0 ; ipt <nHLTmx; ipt++){
          ...
          TH1D *MyHist  = (TH1D*) MC_reco[0][ity][ivar][ipt]->Clone();
          Integralhist(MyHist);
          divBybinWidth(MyHist);
          Myplotset(MyHist,0,0);
...
void SetMycanvas(TCanvas *cpt){
         cpt->SetBorderSize(0);
          cpt->SetRightMargin(0.0);
          cpt->SetTopMargin(0.0); 
   }

void CTLegend( TLegend *legendn ,double  x1,double y1,double x2,double y2,const char* txt){ 
  legendn = new TLegend(x1,y1,x2,y2);
  legendn->SetFillStyle(0);
  legendn->SetBorderSize(0);
  legendn->SetTextSize(1);
  legendn->SetTextFont(42);
  legendn->AddEntry((TObject*)0, txt, "");
}

//void Set2dHist(TH2D *myhist, const char* Title, const char* YTitle, const char* XTitle const char* legend){
     //myhist->

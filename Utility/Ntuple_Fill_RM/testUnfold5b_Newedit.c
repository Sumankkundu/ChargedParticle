...
void testUnfold5b(){  
  ...
  TDirectoryFile *RootHist =new TDirectoryFile("analyzeBasicPat","Nornal root Historgams");
  TDirectoryFile *TUnfoldBinng1D =new TDirectoryFile("analyzeBasicPat1D","TUnfoldBinning 1D Historgams");
...
//--------------------------------------------------------------------------------------
  ...
  TH1* h_recoevtvar_2D[type][nusedvar][nHLTmx]; //Reco
   ...
   TUnfoldBinning *binsRec1D[type][nusedvar][nHLTmx];
   TUnfoldBinning *RecoBinning1D[type][nusedvar][nHLTmx];
   TUnfoldBinning *binsGen1D[type][nusedvar][nHLTmx];
   TUnfoldBinning *GenBinning1D[type][nusedvar][nHLTmx];
   
   //----------------------------------2D Bining using TUnfoldBinning
   TUnfoldBinning *binsRec2D[type][nusedvar][nHLTmx];
   TUnfoldBinning *RecoBinning2D[type][nusedvar][nHLTmx];
   ...
   for(int ity=0; ity <type; ity++){
	 ...
	 sprintf(RecoBinName, "Detector1d_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
         binsRec1D[ity][ivar][ipt] = new TUnfoldBinning(RecoBinName);

         sprintf(RecoBinName, "Recobin1d_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
         RecoBinning1D[ity][ivar][ipt]= binsRec1D[ity][ivar][ipt]->AddBinning(RecoBinName);
	 ...
	 sprintf(RecoBinName, "Recobin2d_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
	 RecoBinning2D[ity][ivar][ipt]= binsRec2D[ity][ivar][ipt]->AddBinning(RecoBinName);
	 
	 sprintf(Axisname, "var_%i", var[ivar]);
	 RecoBinning2D[ity][ivar][ipt]->AddAxis(Axisname,(ity==0) ? rnbinsx0[var[ivar]][ipt] : rnbinsx1[var[ivar]][ipt],(ity==0) ? rbinrngs0[var[ivar]][ipt]: rbinrngs1[var[ivar]][ipt],false,false);
	 sprintf(Axisname, "ht_%i", ipt);
	 RecoBinning2D[ity][ivar][ipt]->AddAxis(Axisname, rhtnbins[ipt], htbins[ipt], htbins[ipt+1], false,false);
       }
     }
   }
   ...
   for(int ity=0; ity <type; ity++){
	 ...
	 sprintf(GenBinName, "Generator1d_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
         binsGen1D[ity][ivar][ipt] = new TUnfoldBinning(GenBinName);

         sprintf(GenBinName, "Genbin1d_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
         GenBinning1D[ity][ivar][ipt]= binsGen1D[ity][ivar][ipt]->AddBinning(GenBinName);

         //sprintf(Axisname, "%s", vartitle[var[ivar]]);
         sprintf(Axisname, "var_%i", var[ivar]);
         GenBinning1D[ity][ivar][ipt]->AddAxis(Axisname,(ity==0) ? nbinsx0[var[ivar]][ipt]: nbinsx1[var[ivar]][ipt],(ity==0) ? binrngs0[var[ivar]][ipt]: binrngs1[var[ivar]][ipt],false,false);
         //--------------------------------------------------------------------
	 sprintf(GenBinName, "Generator2d_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
	 binsGen2D[ity][ivar][ipt] = new TUnfoldBinning(GenBinName);
	 
	 sprintf(GenBinName, "Genbin2d_typ_%i_pt%i_eta0_%i", ity, ipt, var[ivar]);
	 GenBinning2D[ity][ivar][ipt]= binsGen2D[ity][ivar][ipt]->AddBinning(GenBinName);
	 
	 sprintf(Axisname, "var_%i", var[ivar]);
	 GenBinning2D[ity][ivar][ipt]->AddAxis(Axisname,(ity==0) ? nbinsx0[var[ivar]][ipt] : nbinsx1[var[ivar]][ipt],(ity==0) ? binrngs0[var[ivar]][ipt]: binrngs1[var[ivar]][ipt],false,false);
	 sprintf(Axisname, "ht_%i", ipt);
	 GenBinning2D[ity][ivar][ipt]->AddAxis(Axisname, htnbins[ipt], htbins[ipt], htbins[ipt+1],false,false);
       }
     }
   }
   ...
   for(int ity=0; ity <type; ity++){
     for(int ivar=0; ivar < nusedvar ; ivar++){
       for(int ipt = 0 ; ipt < njetptmn ; ipt++){
      	 ...
      	 binsRec1D[ity][ivar][ipt]->PrintStream(cout);
      	 binsRec2D[ity][ivar][ipt]->PrintStream(cout);
	 //----------------------------------------------------------Write Binning
	 Bining1D->cd(); 
	 binsRec1D[ity][ivar][ipt]->Write();   binsGen1D[ity][ivar][ipt]->Write();

         Bining2D->cd();
	 binsRec2D[ity][ivar][ipt]->Write();   binsGen2D[ity][ivar][ipt]->Write();
	 //-----------------------------------------------------------Root Histogram 
	 ...
	 RM_1D[ity][ivar][ipt] = TUnfoldBinning::CreateHistogramOfMigrations(binsRec1D[ity][ivar][ipt], binsGen1D[ity][ivar][ipt], histname ,0,0, Axisname); 
        //----------------------------------------------------------------2D TUnfoldBinng ----------------
         ...
         sprintf(Axisname, "%s", vartitle[var[ivar]]);
         RM_2D[ity][ivar][ipt] = TUnfoldBinning::CreateHistogramOfMigrations(binsRec2D[ity][ivar][ipt], binsGen2D[ity][ivar][ipt], histname ,0,0, Axisname);
       }
     }
   }
 ...
 //Fill The hist
     for(int ity =0; ity< type ; ity++){
       //fill reco
for(int ivar=0; ivar < nusedvar ; ivar++){
       if(isRECO && irecovar[ity]==nusedvar &&  recovar[ity][ivar]>=h_recoevtvar[ity][ivar][irecoht]->GetBinLowEdge(1) && recovar[ity][ivar]<=h_recoevtvar[ity][ivar][irecoht]->GetBinLowEdge(h_recoevtvar[ity][ivar][irecoht]->GetNbinsX()+1)){
       ...
       //fill Gen
       if(isGEN && igenvar[ity]==nusedvar && genvar[ity][ivar]>=h_genevtvar[ity][ivar][irecoht]->GetBinLowEdge(1) && genvar[ity][ivar]<=h_genevtvar[ity][ivar][irecoht]->GetBinLowEdge(h_genevtvar[ity][ivar][irecoht]->GetNbinsX()+1)){
...
//for(int ivar=0; ivar < nusedvar ; ivar++){
       if(isRECO && isGEN && irecovar[ity]==nusedvar && igenvar[ity]==nusedvar && irecoht == igenht && recovar[ity][ivar]>=h_recoevtvar[ity][ivar][irecoht]->GetBinLowEdge(1) && recovar[ity][ivar]<=h_recoevtvar[ity][ivar][irecoht]->GetBinLowEdge(h_recoevtvar[ity][ivar][irecoht]->GetNbinsX()+1) && genvar[ity][ivar]>=h_genevtvar[ity][ivar][irecoht]->GetBinLowEdge(1) && genvar[ity][ivar]<=h_genevtvar[ity][ivar][irecoht]->GetBinLowEdge(h_genevtvar[ity][ivar][irecoht]->GetNbinsX()+1)){
	   int igobbinx1d = RecoBinning1D[ity][ivar][irecoht]->GetGlobalBinNumber(recovar[ity][ivar]);
           int igobbiny1d = GenBinning1D[ity][ivar][irecoht]->GetGlobalBinNumber(genvar[ity][ivar]);
           RM_1D[ity][ivar][irecoht]->Fill(igobbinx1d,igobbiny1d, weight);

	   int igobbinx2d = RecoBinning2D[ity][ivar][irecoht]->GetGlobalBinNumber(recovar[ity][ivar],HT2);
	   int igobbiny2d = GenBinning2D[ity][ivar][irecoht]->GetGlobalBinNumber(genvar[ity][ivar],HT2gen);
	   RM_2D[ity][ivar][irecoht]->Fill(igobbinx2d, igobbiny2d, weight);
	   RM[ity][ivar][irecoht]->Fill(recovar[ity][ivar], genvar[ity][ivar], weight);

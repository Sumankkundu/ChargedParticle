...
*/
   ...
   for(int ity=0; ity <type; ity++){
	 ...
	 RecoBinning[ity][ivar][ipt]->AddAxis(Axisname,(ity==0) ? rnbinsx0[var[ivar]][ipt] : rnbinsx1[var[ivar]][ipt],(ity==0) ? rbinrngs0[var[ivar]][ipt]: rbinrngs1[var[ivar]][ipt], true, true);
	 
	 //sprintf(Axisname, "%s", "H_{T,2}");
	 sprintf(Axisname, "ht_%i", ipt);
	 RecoBinning[ity][ivar][ipt]->AddAxis(Axisname, rhtnbins[ipt], htbins[ipt], htbins[ipt+1], true, true);
       }
     }
   }

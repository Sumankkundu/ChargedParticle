...
//const  char* RefoldEra[5]={"Refold Madgraph(No Regularisation) ","Refold Madgraph(L-Curve scan)","Refold Madgraph(Scan Tau)","Refold Madgraph(ScanSURE)","Refold Pythia8(Iterative method)"};
 ...
 for(int idd=2; idd <3; idd++){
  ...
  TH1D *Unfold[unfold_ty][ntype][nusedvar];
  TH1D *Refold[unfold_ty][ntype][nusedvar];

  TH1D *Ex_Unfold[unfold_ty][ntype][nusedvar];
  TH1D *Ex_Refold[unfold_ty][ntype][nusedvar];


  TH2D *Ex_Unfold_HT[unfold_ty][ntype][nusedvar];
  TH2D *Ex_Refold_HT[unfold_ty][ntype][nusedvar];
  TH2D *Ex_MC_reco_HT[nmc][ntype][nusedvar];
  TH2D *Ex_MC_gen_HT[nmc][ntype][nusedvar];

  TH2D *Corr[unfold_ty][ntype][nusedvar];
  TH2D *Prob[unfold_ty][ntype][nusedvar];
  TH2D *Ematrix[unfold_ty][ntype][nusedvar];


for(int ity=0; ity <ntype; ity++){
      for(int ivar =0;ivar < nusedvar; ivar++){
#ifdef CLOUSER
          sprintf(histname, "%s/%sgen_typ_%i_eta0_%i",Datadirtag[idd],ddtag[idd],ity,var[ivar]);
          Psudo_Data_gen[ity][ivar] =(TH1D*)Unfoldroot->Get(histname);    Psudo_Data_gen[ity][ivar]->Rebin(irbin);
          cout << histname << endl;
#ifdef TRUEAXIS
if(idd >=1){
	  sprintf(histname, "%s/%s%sgen_typ_%i_eta0_%i",Datadirtag[idd],trueAxis[idd],ddtag[idd],ity,var[ivar]);
	  cout << histname << endl;
          if(idd==1){Ex_Psudo_Data_gen[ity][ivar] =(TH1D*)Unfoldroot->Get(histname); }
          if(idd==2){TH2D* temp_Psudo_Data_gen =(TH2D*)Unfoldroot->Get(histname); Ex_Psudo_Data_gen[ity][ivar] = temp_Psudo_Data_gen->ProjectionX();}
	  Ex_Psudo_Data_gen[ity][ivar]->Rebin(irbin);          cout << histname << endl;

          sprintf(histname, "%s/%s%sRecominusfake_%i_eta0_%i",Datadirtag[idd],trueAxis[idd],ddtag[idd],ity,var[ivar]);
          cout << histname << endl;
          if(idd==1){Ex_Psudo_Data_reco_fake[ity][ivar] =(TH1D*)Unfoldroot->Get(histname); }
          if(idd==2){TH2D* temp_Psudo_reco_fake =(TH2D*)Unfoldroot->Get(histname); Ex_Psudo_Data_reco_fake[ity][ivar] = temp_Psudo_reco_fake->ProjectionX();}
          Ex_Psudo_Data_reco_fake[ity][ivar]->Rebin(irbin);          cout << histname << endl;

          sprintf(histname, "%s/%s%sGenminusmiss_%i_eta0_%i",Datadirtag[idd],trueAxis[idd],ddtag[idd],ity, var[ivar]);
          cout << histname << endl;
          if(idd==1){Ex_Psudo_Data_gen_miss[ity][ivar] =(TH1D*)Unfoldroot->Get(histname); }
          if(idd==2){TH2D* temp_Psudo_gen_miss =(TH2D*)Unfoldroot->Get(histname); Ex_Psudo_Data_gen_miss[ity][ivar] = temp_Psudo_gen_miss->ProjectionX();}
          Ex_Psudo_Data_gen_miss[ity][ivar]->Rebin(irbin);          cout << histname << endl;
}
#endif
#endif
      }
  ...
  for(int  imc =0; imc < nmc ; imc++){
    for(int ity=0; ity <ntype; ity++){
      for(int ivar =0 ; ivar < nusedvar ; ivar++){
	  sprintf(histname, "%s/%sgen_typ_%i_eta0_%i",dirname[imc],ddtag[idd], ity,var[ivar]); 
	  MC_gen[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);
	  MC_gen[imc][ity][ivar]->Rebin(irbin);
	  
	  sprintf(histname, "%s/%sreco_typ_%i_eta0_%i", dirname[imc],ddtag[idd], ity,  var[ivar]); 
	  MC_reco[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);	  cout << histname << endl;

	  //Fake and Miss corrected Reco and Gen
          sprintf(histname, "%s/%sGenminusmiss_%i_eta0_%i",dirname[imc],ddtag[idd], ity, var[ivar]);
          MC_gen_miss[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);     MC_gen_miss[imc][ity][ivar]->Rebin(irbin);

          sprintf(histname, "%s/%sRecominusfake_%i_eta0_%i", dirname[imc],ddtag[idd], ity, var[ivar]);
          MC_reco_fake[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);       cout << histname << endl;
#ifdef TRUEAXIS
if(idd>=1){
          sprintf(histname, "%s/%s%sgen_typ_%i_eta0_%i",dirname[imc],trueAxis[idd],ddtag[idd], ity,var[ivar]); 
          if(idd==1){Ex_MC_gen[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);}     
          if(idd==2){TH2D* temp_MC_gen = (TH2D*) Unfoldroot->Get(histname);Ex_MC_gen_HT[imc][ity][ivar]= (TH2D*)temp_MC_gen->Clone();Ex_MC_gen[imc][ity][ivar] = temp_MC_gen->ProjectionX();} 
	  Ex_MC_gen[imc][ity][ivar]->Rebin(irbin);

          sprintf(histname, "%s/%s%sreco_typ_%i_eta0_%i", dirname[imc], trueAxis[idd],ddtag[idd], ity, var[ivar]);
          if(idd==1){Ex_MC_reco[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname); cout << histname << endl;}
          if(idd==2){TH2D* temp_MC_reco = (TH2D*) Unfoldroot->Get(histname);   Ex_MC_reco[imc][ity][ivar] = temp_MC_reco->ProjectionX();  cout << histname << endl;}
      

          sprintf(histname, "%s/%s%sGenminusmiss_%i_eta0_%i",dirname[imc],trueAxis[idd],ddtag[idd], ity, var[ivar]);
          if(idd==1){Ex_MC_gen_miss[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);}
          if(idd==2){TH2D* temp_MC_gen_miss = (TH2D*) Unfoldroot->Get(histname); Ex_MC_gen_miss[imc][ity][ivar] = temp_MC_gen_miss->ProjectionX();    }
          Ex_MC_gen_miss[imc][ity][ivar]->Rebin(irbin);

          sprintf(histname, "%s/%s%sRecominusfake_%i_eta0_%i", dirname[imc], trueAxis[idd],ddtag[idd], ity, var[ivar]);
          if(idd==1){Ex_MC_reco_fake[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname); cout << histname << endl;}
          if(idd==2){TH2D* temp_MC_reco_fake = (TH2D*) Unfoldroot->Get(histname);   Ex_MC_reco_fake[imc][ity][ivar] = temp_MC_reco_fake->ProjectionX();  cout << histname << endl;}
          }
#endif
	  //-------------------------------------Read Reso--------------------------------------
	  sprintf(histname, "%s/%scorr_typ_%i_eta0_%i",dirname[imc], ddtag[idd],ity, var[ivar]); 
	  MC_Res[imc][ity][ivar] = (TH2D*) Unfoldroot->Get(histname);	  cout << histname << endl;
	  
	  //------------------------------------------------------
	  sprintf(histname,"%s/%smiss_rate_%i_eta0_%i",dirname[imc],ddtag[idd], ity, var[ivar]);
	  hist_eff[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);  cout << histname << endl;
	  sprintf(histname,"%s/%sfake_rate_%i_eta0_%i",dirname[imc],ddtag[idd], ity, var[ivar]);
	  hist_fake[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname); cout << histname << endl;
	  sprintf(histname,"%s/%sPurity_%i_eta0_%i", dirname[imc],ddtag[idd], ity, var[ivar]);
	  hist_purity[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname); cout << histname << endl;
	  sprintf(histname,"%s/%sstability_%i_eta0_%i", dirname[imc],ddtag[idd], ity, var[ivar]);
	  hist_stbl[imc][ity][ivar] = (TH1D*) Unfoldroot->Get(histname); cout << histname << endl;
  ...
  for(int ity=0; ity <ntype; ity++){
    for(int ivar =0 ; ivar < nusedvar ; ivar++){
	sprintf(histname, "%s/%sreco_typ_%i_eta0_%i",Datadirtag[idd], ddtag[idd], ity, var[ivar]); //reco_typ_1_pt6_eta0_15
	Data_reco[ity][ivar] = (TH1D*) Unfoldroot->Get(histname);	cout << histname << endl;     

#ifdef TRUEAXIS
if(idd>=1){
        sprintf(histname, "%s/%s%sreco_typ_%i_eta0_%i",Datadirtag[idd], trueAxis[idd],ddtag[idd], ity,var[ivar]);
        if(idd==1){Ex_Data_reco[ity][ivar] = (TH1D*) Unfoldroot->Get(histname);  cout << histname << endl;}
        if(idd==2){TH2D* temp_Data_reco = (TH2D*) Unfoldroot->Get(histname); Ex_Data_reco[ity][ivar] = temp_Data_reco->ProjectionX();  cout << histname << endl;}
}
#endif
    }
  }
  ...
  for(int iun=0; iun < unfold_ty; iun++){
    for(int ity=0; ity <ntype; ity++){
      for(int ivar=0; ivar < nusedvar ; ivar ++){
	  sprintf(histname, "%s/%s%s%i_eta0_%i",dirtag[idd] ,ddtag[idd],regN[iun],ity,var[ivar]); 
	  Unfold[iun][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);	  cout << histname << endl;
#ifdef TRUEAXIS
if(idd>=1){
          sprintf(histname, "%s/%s%s%s%i_eta0_%i",dirtag[idd], trueAxis[idd],ddtag[idd],regN[iun], ity, var[ivar]);
          if(idd==1){Ex_Unfold[iun][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);   cout << histname << endl;}
          if(idd==2){TH2D* temp_Unfold = (TH2D*) Unfoldroot->Get(histname); Ex_Unfold_HT[iun][ity][ivar]= (TH2D*)temp_Unfold->Clone(); Ex_Unfold[iun][ity][ivar] = temp_Unfold->ProjectionX();  cout << histname << endl;}
	}
#endif
	  sprintf(histname, "%s/%s%s%i_eta0_%i",dirtag[idd] ,ddtag[idd],CORR[iun],ity, var[ivar]); 
          Corr[iun][ity][ivar]=(TH2D*) Unfoldroot->Get(histname);
	  sprintf(histname, "%s/%s%s%i_eta0_%i",dirtag[idd] ,ddtag[idd],COVN[iun],ity,var[ivar]); 
           Ematrix[iun][ity][ivar]= (TH2D*) Unfoldroot->Get(histname);
	  sprintf(histname, "%s/%s%s%i_eta0_%i",dirtag[idd] ,ddtag[idd],ProbN[iun],ity,var[ivar]); 
	   Prob[iun][ity][ivar] = (TH2D*) Unfoldroot->Get(histname);
      }
    } //unfolded histUnfold
  }
//------------------------------------------------------------//Read Folded Back
  for(int iun=0; iun < unfold_ty; iun++){
    for(int ity=0; ity <ntype; ity++){
      for(int ivar=0; ivar < nusedvar ; ivar ++){
	  sprintf(histname, "%s/%s%s%i_eta0_%i",dirtag[idd] ,ddtag[idd],reg_refold[iun],ity, var[ivar]); 
	  Refold[iun][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);	  cout << histname << endl;
#ifdef TRUEAXIS
if(idd>=1){
          sprintf(histname, "%s/%s%s%s%i_eta0_%i",dirtag[idd], trueAxis[idd],ddtag[idd],reg_refold[iun], ity, var[ivar]);
           if(idd==1){Ex_Refold[iun][ity][ivar] = (TH1D*) Unfoldroot->Get(histname);  cout << histname << endl;}
           if(idd==2){TH2D* temp_Refold = (TH2D*) Unfoldroot->Get(histname); Ex_Refold_HT[iun][ity][ivar]= (TH2D*)temp_Refold->Clone(); Ex_Refold[iun][ity][ivar] = temp_Refold->ProjectionX();  cout << histname << endl;}
      }
#endif
      }
    } //unfolded histUnfold
...
//------------------------- Reco Projection comparison-----------------------
  for(int ity=0; ity <ntype; ity++){
    for(int ivar =0;ivar < nusedvar; ivar++){
      sprintf(histname, "%s/%s%s%i_eta0_%i",dirname[umc],ddtag[idd],"ProjectX_",ity, var[ivar]);    TH1* RMx=(TH1D*) Unfoldroot->Get(histname);
      sprintf(histname, "%s/%s%s%i_eta0_%i",dirname[umc],ddtag[idd],"Recominusfake_",ity, var[ivar]);    TH1* RecoFake=(TH1D*) Unfoldroot->Get(histname);
      ...
      Myplotset(MyHist,0,0);
        ...
        //float ratio_range1[2]={1.2,0.9};
        int num1[2]={imc,1} ;
        float lpos1[7] ={.32,0.2,0.55,0.38, .04, 1.05,0.95};
        ...
        if(ity==0 && ivar==0){cpt9->Print(pdfname,"pdf");
        }else if(ity==1 && ivar==4) {cpt9->Print(pdfname2,"pdf");
...
//--------------------------Gen Projection comparison------------------
  for(int ity=0; ity <ntype; ity++){
    for(int ivar =0 ; ivar < nusedvar ; ivar++){
      sprintf(histname, "%s/%s%s%i_eta0_%i",dirname[umc],ddtag[idd],"ProjectY_",ity, var[ivar]);  TH1* RMx=(TH1D*) Unfoldroot->Get(histname);
      sprintf(histname, "%s/%s%s%i_eta0_%i",dirname[umc],ddtag[idd],"Genminusmiss_",ity, var[ivar]); TH1* GenMiss=(TH1D*) Unfoldroot->Get(histname);
      cpt9->cd();
...
//--------------------------------------------------------------
  for(int ity=0; ity <ntype; ity++){
    for(int ivar =0 ; ivar < nusedvar ; ivar++){
      ...
      sprintf(histname, "%s/%s%s%i_eta0_%i",foldtag[idd],ddtag[idd],"Fold_",ity, var[ivar]);  TH1D* GenFold=(TH1D*)Unfoldroot->Get(histname);
        ...
        //MC_input[iout]=(TH1D*) Unfoldroot->Get(histname);   //Genfold  are already corrected for fake 
          MC_input[iout] = (TH1D*) MC_reco[umc][ity][ivar]->Clone(); 
          Integralhist(MC_input[iout]);
         // divBybinWidth(MC_input[iout]);
...
//--------------------------------------------------------------
  for(int ity=0; ity <ntype; ity++){
    for(int ivar =0 ; ivar < nusedvar ; ivar++){
      ...
      TH1D *GenFold;
      sprintf(histname, "%s/E%s%s%i_eta0_%i",foldtag[idd],ddtag[idd],"Fold_",ity, var[ivar]); cout <<histname<< endl; 
      ...
      if(idd==2){TH2D* GenFold2D=(TH2D*)Unfoldroot->Get(histname); GenFold = GenFold2D->ProjectionX();}
        ...
        const char *data_index[1];
        for(int iout = 0 ; iout < imc ; iout++){
        sprintf(histname, "Pythia8/%s%s%i_eta0_%i",ddtag[idd],"Recominusfake_",ity, var[ivar]);
        //MC_input[iout]=(TH1D*) Unfoldroot->Get(histname);
        MC_input[iout] = (TH1D*) Ex_MC_reco[iout][ity][ivar]->Clone();
        Integralhist(MC_input[iout]);
        //divBybinWidth(MC_input[iout]);
        MCinput_index[iout]= "Pythia RECO"; }
        data_index[0]= "Folded";
...
//	MC_Res[imc][ity][ivar]->RebinY(irbin);
        ...
        if(idd==1){sprintf(lplot_xtitle, "Reco bin ID ( %s)",Esvlogx[ivar]); sprintf(lplot_ytitle, "Gen  bin ID ( %s) ",Esvlogx[ivar]);}	
        if(idd==2){sprintf(lplot_xtitle, "Reco bin ID ( %s, H_{T,2})",Esvlogx[ivar]); sprintf(lplot_ytitle, "Gen  bin ID ( %s, H_{T,2}) ",Esvlogx[ivar]);}	
...
//----------------------------------------All Unfold in one plot(Closure Test)
    for(int ity=0; ity <ntype; ity++){
	  ...
	  TH1D *MyHist  = (TH1D*) Unfold[0][ity][ivar]->Clone();
...
#ifdef CLOUSER
          unfold_input[iout]  = (TH1D*) Psudo_Data_gen[ity][ivar]->Clone();
#else
          unfold_input[iout] = (TH1D*) MC_gen[umc][ity][ivar]->Clone();
#endif
      ...
      for(int iout = 0 ; iout < unfold_ty ; iout++){
          ...
          char lplot_xtitle[100];
          sprintf(lplot_xtitle, "%s",Esvlogx[ivar]);
          if(idd==1){sprintf(lplot_xtitle, " bin ID ( %s)",Esvlogx[ivar]);}
          if(idd==2){sprintf(lplot_xtitle, " bin ID ( %s, H_{T,2})",Esvlogx[ivar]);}
          //float ratio_range1[2]={1.2,0.9};
...
if(idd>=1){
   for(int ity=0; ity <ntype; ity++){
	  ...
	  TH1D *MyHist  = (TH1D*) Ex_Unfold[0][ity][ivar]->Clone();    	  cout << MyHist->GetName() << endl;
	  ...
	  Integralhist(MyHist);
         // divBybinWidth(MyHist);
          ...
          for(int iout = 0 ; iout < unfold_ty ; iout++){
#ifdef CLOUSER
          unfold_input[iout]  = (TH1D*) Ex_Psudo_Data_gen[ity][ivar]->Clone();
#else
          unfold_input[iout] = (TH1D*) Ex_MC_gen[umc][ity][ivar]->Clone();
#endif
          //unfold_input[iout] = (TH1D*) Unfold[iout][ity][ivar]->Clone();
          //MC_input[iout]->Rebin(2);
          Integralhist(unfold_input[iout]);
      ...
      //    if(ity==1 && ivar==2 && ipt ==0){lpos1[0] =.45; lpos1[1]=0.65; lpos1[2]=0.8; lpos1[3]=0.9;}
          ...
          sprintf(pdfname,"%sTrueunfold_plot_%i.pdf(", ddtag[idd],1234);sprintf(pdfname1,"%sTrueunfold_plot_%i.pdf",ddtag[idd],1234);sprintf(pdfname2,"%sTrueunfold_plot_%i.pdf)",ddtag[idd],1234);
          if(ity==0 && ivar==0){cpt0->Print(pdfname,"pdf");
          }else if(ity==1 && ivar==4) {cpt0->Print(pdfname2,"pdf");
          }else{cpt0->Print(pdfname,"pdf");};
     }
  ...
  for(int iun=0; iun < unfold_ty; iun++){
    for(int ity=0; ity <ntype; ity++){
	  ...
	  sprintf(pdfname, "%sRefold_plot_%i.pdf(" ,ddtag[idd],iun); sprintf(pdfname1, "%sRefold_plot_%i.pdf" ,ddtag[idd],iun);sprintf(pdfname2, "%sRefold_plot_%i.pdf)" ,ddtag[idd],iun);
	  if(ity==0 && ivar==0 ){cpt0->Print(pdfname,"pdf");
	  }else if(ity==1 && ivar==4) {cpt0->Print(pdfname2,"pdf");
	  }else{cpt0->Print(pdfname,"pdf");};
         } 
    ...
    for(int ity=0; ity <ntype; ity++){
      for(int ivar =0 ; ivar < nusedvar ; ivar++){
	   ...
	   Refold_input[iout] = (TH1D*) Refold[iout][ity][ivar]->Clone();
          ...
          sprintf(pdfname, "%sRefold_plot_%i.pdf(",ddtag[idd], 1234); sprintf(pdfname1, "%sRefold_plot_%i.pdf" ,ddtag[idd],1234);sprintf(pdfname2, "%sRefold_plot_%i.pdf)",ddtag[idd],1234);
    ...
    for(int ity=0; ity <ntype; ity++){
      for(int ivar =0 ; ivar < nusedvar ; ivar++){
         ...
         // divBybinWidth(MyHist);
          ...
          sprintf(pdfname, "%sTrueRefold_plot_%i.pdf(",ddtag[idd], 1234); sprintf(pdfname1, "%sTrueRefold_plot_%i.pdf" ,ddtag[idd],1234);sprintf(pdfname2, "%sTrueRefold_plot_%i.pdf)",ddtag[idd],1234); 
          if(ity==0 && ivar==0 ){cpt0->Print(pdfname,"pdf");
          }else if(ity==1 && ivar==4 ) {cpt0->Print(pdfname2,"pdf");
          }else{cpt0->Print(pdfname,"pdf");};
...
//MC_HT2[0]->Draw();
      ...
      for(int iout = 0 ; iout < unfold_ty ; iout++){
        ...
        //  CMS_lumi( cpt0, iPeriod, iPos ); cpt0->Update();
          ...
          sprintf(pdfname,"%sHTunfold_plot_%i.pdf(", ddtag[idd],1234);sprintf(pdfname1,"%sHTunfold_plot_%i.pdf",ddtag[idd],1234);sprintf(pdfname2,"%sHTunfold_plot_%i.pdf)",ddtag[idd],1234);
          if(ity==0 && ivar==0 && ipt ==0){cpt0->Print(pdfname,"pdf");
          }else if(ity==1 && ivar==4 && ipt==7) {cpt0->Print(pdfname2,"pdf");
          }else{cpt0->Print(pdfname,"pdf");};
...
MC_HT2[0]->Draw();
	  ...
	  //cpt0 =(TCanvas*)(ratio_canV2(num1, lpos1, MyHist, unfold_input, lplot_xtitle,MCinput_index,data_index));
          ...
          sprintf(pdfname,"%sHT_plot_%i.pdf(", ddtag[idd],1234);sprintf(pdfname1,"%sHT_plot_%i.pdf",ddtag[idd],1234);sprintf(pdfname2,"%sHT_plot_%i.pdf)",ddtag[idd],1234);

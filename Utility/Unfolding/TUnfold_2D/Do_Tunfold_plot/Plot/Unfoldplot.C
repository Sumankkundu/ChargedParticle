...
void Unfoldplot(){
  ...
  //Int_t HT2range[nHLTmx+1]={84, 111, 172, 241, 309, 376, 462, 568, 3000}; // 2017 1June21 UL17
 //Int_t HT2range[nHLTmx+1]={83, 109, 172, 241, 309, 377, 462, 570, 3000}; //2017
  Int_t HT2range[nHLTmx+1]={83, 109, 176, 247, 318, 387, 477, 573, 3000}; //2018 25May21 UL18
...
#ifdef CLOUSER
  const  char* mcname[4]={"Py8 Gen","Py8Flat Gen","MadGraph Gen","Herwig7 Gen"};
#else 
  ...
  Unf_root[0] = TFile::Open("../Unfold/Unfolded_Result_PY8_PY8.root");  // Py- py
  Unf_root[1] = TFile::Open("../Unfold/Unfolded_Result_Py8_Flat_PY8.root");  // Py -Py flat 
  Unf_root[2]= TFile::Open("../Unfold/Unfolded_Result_MG_PY8.root");  // Py MG 
  Unf_root[3]= TFile::Open("../Unfold/Unfolded_Result_HW_Py8.root");  // Py MG 
  ...
  Unf_dataCT[0]= TFile::Open("../Unfold/Unfolded_Result_HW_Py8.root");  // Py MG 
  Unf_dataCT[1]= TFile::Open("../Unfold/Unfolded_Result_HW_Py8.root");  // Py MG 
  Unf_dataCT[2]= TFile::Open("../Unfold/Unfolded_Result_HW_Py8.root");  // Py MG
  
  Unf_err[0]= TFile::Open("stat_Result.root");  //Stat Unc
  Unf_err[1]= TFile::Open("Total_unc.root");  // Total Unc
  ...
  TCanvas *ratio_canV3(int Nplot[3],float plegend[8], TH1D* data, TH1D* MC[Nplot[0]], char* lowpadx, const char* modnam[Nplot[0]], const  char* datanm[3], TH1D* Terr[2]);
  ...
  TH1D *Totalerr_HT[ntype][nusedvar][nHLTmx];
  TH1D *Staterr_HT[ntype][nusedvar][nHLTmx];
...
for(int ity=0; ity <ntype; ity++){
	  ...
	  Data_reco[ity][ivar] = (TH1D*)ReadHist1D(Datadir+"/dd_reco_typ_"+ to_string(ity)+"_eta0_"+to_string(var[ivar]),Unfoldroot);
          ...
          Staterr_HT[ity][ivar][ipt] = (TH1D*)ReadHist1D("stat_erro_"+ to_string(ity)+"_eta0_"+to_string(var[ivar])+"_pt"+to_string(ipt),Unf_err[0]);             
	  Totalerr_HT[ity][ivar][ipt] = (TH1D*)ReadHist1D("total_erro_"+ to_string(ity)+"_eta0_"+to_string(var[ivar])+"_pt"+to_string(ipt),Unf_err[1]);             
...
//------------------------------------------------------Efficiency, Purity,Fake rate, stability
  ...
  for(int ity=0; ity <ntype; ity++){
	...
	//void SetMycanvas(TCanvas *cpt,double bs,double lm, double rm, double tm,double bm){
   	...
   	TLegend *leg2 = new TLegend(0.4,0.6,0.7,.99);
	CTLegend(leg2," ","");
	...
	if(ipt<=6){sprintf(Title,"%i < H_{T,2} < %i %s", HT2range[ipt] , HT2range[ipt+1] ,"GeV/c" );}
        ...
        for (int i = 1; i <= hist_effHT[umc][ity][ivar][ipt]->GetNbinsX(); ++i) {
         ...
         hist_effHT[umc][ity][ivar][ipt]->SetMinimum(-0.01); hist_effHT[umc][ity][ivar][ipt]->SetMaximum(1.01);
  ...
  for(int iun=0; iun < unfold_ty; iun++){
    for(int ity=0; ity <ntype; ity++){
	 ...
	 TCanvas *cpt7 = new TCanvas("cpt7", "canvas7", 600,600 );  //for  
	  ...
	  TH1D *MC_input[nmc];
	  TH1D *err_input[2];
	  err_input[0] = (TH1D*) Staterr_HT[ity][ivar][ipt]->Clone();
	  err_input[1] = (TH1D*) Totalerr_HT[ity][ivar][ipt]->Clone();

	  const char *MCinput_index[nmc], *data_index[3];
	  for(int iout = 0 ; iout < nmc ; iout++){
	  ...
	  MCinput_index[iout]= mcname[iout]; }
          ...
          //int num1[2]={nmc,1} ;
          int num1[3]={nmc,1,0} ;
          float lpos1[8] ={.32,0.15,0.55,0.45, .055, 1.45,0.7,0.14};
   	  ...
   	  cpt0 =(TCanvas*)(ratio_canV3(num1, lpos1, MyHist, MC_input, lplot_xtitle,MCinput_index,data_index, err_input));
          SetMycanvas(cpt0,0,0.1,0.02,0.05,0.12);
          CMS_lumi( cpt0, iPeriod, iPos ); cpt0->Update();
       ...
       cpt5->cd();
       double titoff1[3]={1.2,1.3,1.5};
       double titsize1[3] ={0.035,0.035,0.040};
       SetMycanvas(cpt5,0,0.1,0.17,0.05,0.1);
       gStyle->SetPaintTextFormat( "4.2f");
       sprintf(lplot_xtitle, "Gen bin ID ( %s, H_{T,2})",Esvlogx[ivar]); sprintf(lplot_ytitle, "Gen  bin ID ( %s, H_{T,2}) ",Esvlogx[ivar]);
       Set2dHist(Corr[ity][ivar],lplot_xtitle, lplot_ytitle,"Correlation Coefficients", titoff1, titsize1);
       Corr[ity][ivar]->Draw("colz ");
       //Corr[iun][ity][ivar]->Draw("colz text");

       TLegend *leg1 = new TLegend(0.05,0.6,0.4,0.8);
       CTLegend(leg1,"Unfolded with Pythia8",Title); leg1->AddEntry((TObject*)0,itypeN[ity] , ""); leg1->AddEntry((TObject*)0,Unfoldtype[iun] , "");leg1->SetTextColor(-8);
       //leg1->Draw();
       ...
       cpt6->cd();
       SetMycanvas(cpt6,0,0.1,0.17,0.05,0.1);
       sprintf(lplot_xtitle, "Gen bin ID ( %s, H_{T,2})",Esvlogx[ivar]); sprintf(lplot_ytitle, "Reco  bin ID ( %s, H_{T,2}) ",Esvlogx[ivar]);
       Set2dHist(Prob[ity][ivar],lplot_xtitle, lplot_ytitle,"Probability",titoff1, titsize1);
       ...
       cpt7->cd();
       SetMycanvas(cpt7,0,0.1,0.17,0.05,0.1);
       sprintf(lplot_xtitle, "Gen bin ID ( %s, H_{T,2})",Esvlogx[ivar]); sprintf(lplot_ytitle, "Gen  bin ID ( %s, H_{T,2}) ",Esvlogx[ivar]);
       Set2dHist(Ematrix[ity][ivar],lplot_xtitle, lplot_ytitle,"",titoff1, titsize1);
...
//  data->SetFillStyle(1111);
  ...
  int  color[40] = {2,58,6,46,8,49,1,41,42,30,46,28,29,38,30,12,37,49,9,32,2,3,6,7,8,49,1,41,42,30,46,28,29,38,30,12,37,49,9,32};
  int   style[40]={1,2,5,4,5,6,7,8,9,9,2,3,4,5,6,7,8,9,9,9,1,2,3,4,5,6,7,8,9,9,2,3,4,5,6,7,8,9,9,9};
  for(int iup =0; iup < Nplot[0] ; iup++){
    MC[iup]->SetLineStyle(style[iup]);
    MC[iup]->SetLineColor(color[iup]);
    MC[iup]->SetLineWidth(3);
...
TCanvas *ratio_canV3(int Nplot[3],float plegend[8], TH1D* data, TH1D* MC[Nplot[0]], char* lowpadx, const char* modnam[Nplot[0]], const  char* datanm[3], TH1D* Terr[2]){
  ...
  //--------------------------------------------------------------handel error
   TH1D *data_stat = (TH1D*)data->Clone(); data_stat->Reset();
   TH1D *total_stat = (TH1D*)data->Clone(); data_stat->Reset();
   TH1D *unit_hist = (TH1D*)data->Clone(); unit_hist->Reset();

   for(int ierr=1; ierr < total_stat->GetNbinsX()+1; ierr++){
     double sss = Terr[0]->GetBinContent(ierr);
     double ttt = Terr[1]->GetBinContent(ierr);

    data->SetBinError(ierr,(ttt*data->GetBinContent(ierr)));
    //data->SetBinError(ierr,ttt);

    unit_hist->SetBinContent(ierr,1.0);
    unit_hist->SetMarkerSize(0);
    unit_hist->SetLineColor(kBlack);
    unit_hist->SetLineWidth(1);

    data_stat->SetBinContent(ierr,1.0);
    total_stat->SetBinContent(ierr,1.0);
    data_stat->SetBinError(ierr,sss);
    total_stat->SetBinError(ierr,ttt);


    total_stat->SetMaximum(plegend[5]);  // .. range
    total_stat->SetMinimum(plegend[6]);  // Define Y ..
    data_stat->SetMinimum(plegend[6]);  // Define Y ..
    data_stat->SetMaximum(plegend[5]);  // .. range

    total_stat->SetLineWidth(1);
    total_stat->SetLineColor(kBlack);
    total_stat->SetFillColor(kYellow);
    total_stat->SetMarkerSize(0);
    total_stat->SetFillStyle(1111);

    data_stat->SetLineWidth(1);
    data_stat->SetLineColor(kBlack);
    data_stat->SetFillColor(21);
    data_stat->SetMarkerSize(0);
    data_stat->SetFillStyle(1111);
  }
//----------------------------------------------------
...
//  data->SetMarkerSize(0);
 ...
 //------------------------------------------Stat Error(DATA)
   TH1D *datastaterr = (TH1D*)data->Clone("staterr"); datastaterr->Reset();
    ...
    //gStyle->SetErrorX(0.5);
   // gStyle->IsTransparent();
    datastaterr->SetFillColor(28);
    datastaterr->SetMarkerSize(0.1);
    //datastaterr->SetFillStyle(3002);
    datastaterr->SetFillStyle(1001);
    //datastaterr->SetFillColor(21);
    //datastaterr->SetMarkerSize(0.1);

   //--------------------------------------------------------------handel error

  TLegend *legendn = new TLegend(plegend[0],plegend[1],plegend[2],plegend[3],"","brNDC");
  legendn->SetFillColor(10);
  legendn->SetFillStyle(0);
  legendn->SetBorderSize(0);
  legendn->SetTextSize(plegend[4]);
  legendn->SetTextFont(42);
  ...
  //if(isstat)legendn->AddEntry(datastaterr, "Statistical Uncertainty","f"); //Stat unc
  if(isstat)legendn->AddEntry(data_stat, "Statistical Uncertainty","f"); //Stat unc
  if(istot)legendn->AddEntry(total_stat, "Total Uncertainty","f"); //Stat unc
  legendn->Draw();
  HT_range->Draw();
...
if(ilow==0){
   ...
   if(istot) total_stat->Draw("E2 Same  ");
   if(isstat) data_stat->Draw("E2 Same");
    unit_hist->Draw("Same");
   }
